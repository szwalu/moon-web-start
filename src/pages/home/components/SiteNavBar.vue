...
function handleCateClick(cateIndex: number) {
  ...
  if (clickedCate.groupList?.length > 0)
...
const subMenuRows = computed(() => {
  const cate = siteStore.data[activeSubMenuIndex.value]
  return cate?.groupList?.length ? [cate.groupList] : []
})
...
<template>
  <section class="site-navbar-sidebar pb-12 text-16" :class="{ 'is-open': settingStore.isSideNavOpen }">
