...
// === IME BAR PATCH v3 (fixed for lint) ===
// === IME BAR PATCH v3 (fixed for lint) ===
const kbBarRef = ref<HTMLElement | null>(null)
const kbVisible = ref(false)
const kbStyle = ref<Record<string, string>>({
  position: 'fixed',
  top: '0px',
  left: '0px',
  width: '0px',
...
const isMobileEnv = _isTouch || /iphone|ipad|ipod|android/i.test(_ua)

// 不直接引用 textarea 变量，改为通过元素类型 + class 判断，避免 ts/no-use-before-define
function _isEditorTarget(el: EventTarget | null) {
  return !!(el instanceof HTMLTextAreaElement && el.classList.contains('editor-textarea'))
}
...
function positionKB() {
  ...
  const bar = kbBarRef.value
  const vv = (window as any).visualViewport as VisualViewport | undefined
  const barH = bar?.offsetHeight || 44
  const margin = 6

  if (vv) {
    const top = Math.round(vv.offsetTop + vv.height - barH - margin)
    const left = Math.round(vv.offsetLeft + 8)
    const width = Math.round(vv.width - 16)
    ...
    const w = Math.max(160, Math.min(window.innerWidth - 16, 640))
    kbStyle.value = {
      position: 'fixed',
      top: `${window.innerHeight - barH - margin}px`,
      left: '8px',
      width: `${w}px`,
    }
  }
...
// 这些一行函数改为多行，避免 style/max-statements-per-line
...
onMounted(() => {
  ...
  window.addEventListener('focusin', _onFocusIn, { capture: true })
  window.addEventListener('focusout', _onFocusOut, { capture: true })
  ...
  // 不直接读 textarea 变量；通过 activeElement 的类型和 class 来判断
  const ae = document.activeElement
  if (ae instanceof HTMLTextAreaElement && ae.classList.contains('editor-textarea')) {
    showKB()
    _onEditorInputBump()
  }

  // 给当前编辑的 textarea 绑 input 监听
  const ta = document.querySelector('textarea.editor-textarea') as HTMLTextAreaElement | null
  if (ta)
    ta.addEventListener('input', _onEditorInputBump)
})
...
onUnmounted(() => {
  ...
  window.removeEventListener('focusin', _onFocusIn as any, { capture: true } as any)
  window.removeEventListener('focusout', _onFocusOut as any, { capture: true } as any)

  const vv = (window as any).visualViewport as VisualViewport | undefined
  if (vv) {
    vv.removeEventListener('resize', _onVVResize as any)
    vv.removeEventListener('scroll', _onVVScroll as any)
  }

  window.removeEventListener('scroll', _onWinScroll as any, true)
  window.removeEventListener('resize', _onWinResize as any)

  const ta = document.querySelector('textarea.editor-textarea') as HTMLTextAreaElement | null
  if (ta)
    ta.removeEventListener('input', _onEditorInputBump as any)
})
// === IME BAR PATCH v3 end ===
// === IME BAR PATCH v3 end ===
...
<template>
  ...
  <!-- === IME 工具条（独立 Teleport，不影响你原有 footer） === -->
  <teleport to="body">
    <div
      v-if="kbVisible"
      ref="kbBarRef"
...
/* === IME 工具条样式（不与现有类名冲突） === */
.kb-bar {
  /* 几何定位由内联 style 提供（top/left/width） */
  z-index: 9999;
  display: flex;
  gap: 10px;
  align-items: center;
